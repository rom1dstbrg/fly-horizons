...
import { CalendarRange, Ban, Plus, X, Check } from "lucide-react";
...
export function PiloteDisponibilitesClient({
  ...
  return (
    <form onSubmit={handleSave} className="space-y-4 max-w-2xl">
      <div className="bg-card border border-navy/15 rounded-[10px] p-5">
        <FormSection
          title={<span className="flex items-center gap-1.5 normal-case text-sm font-semibold text-foreground tracking-normal"><CalendarRange size={14} /> Période récurrente</span>}
          description="Rien d'indiqué = vos annonces restent ouvertes à n'importe quelle date (comportement par défaut)."
        >
          <div className="space-y-3">
            ...
            {plageDebut && plageFin && (
              <>
                <FormGrid cols={2}>
                  <FormField label="Disponible à partir de">
                ...
                <FormField label="Jours de la semaine" hint="Aucun coché = tous les jours de la période.">
      ...
      <div className="bg-card border border-navy/15 rounded-[10px] p-5">
        <FormSection
          title={<span className="flex items-center gap-1.5 normal-case text-sm font-semibold text-foreground tracking-normal"><Ban size={14} /> Exceptions ponctuelles</span>}
          description="Une date fermée malgré la période ci-dessus, ou au contraire un jour libre en plus."
        >
          <div className="space-y-3">
