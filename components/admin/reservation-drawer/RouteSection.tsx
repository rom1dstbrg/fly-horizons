  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2 gap-2">
        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[1.5px] flex items-center gap-1.5 shrink-0">
          <MapPin size={11} />
          Route
        </p>
        <div className="flex items-center gap-1.5 min-w-0">
          {routeStatusCfg && (
            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border shrink-0 ${routeStatusCfg.color}`}>
          ...
          <button
            ...
            className="flex items-center gap-1 px-2 py-1 rounded-md text-[11px] font-semibold text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors cursor-pointer min-w-0"
          >
            <Navigation size={11} className="text-primary shrink-0" />
            <span className="truncate">Itinéraires</span>
            <ChevronDown size={10} className="shrink-0" />
          </button>
          <button
            onClick={onFullscreen}
            title="Agrandir la carte"
            className="p-1 rounded hover:bg-secondary transition-colors text-muted-foreground hover:text-foreground cursor-pointer shrink-0"
          >
            <Maximize2 size={12} />
          </button>
        </div>
