...
function CopyCode({ code }: { code: string }) {
  ...
  return (
    <button onClick={copy} className="flex items-center gap-1.5 font-mono text-sm font-bold text-primary hover:opacity-70 transition-opacity" title="Copier">
      {code}
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div>
            ...
            <input name="duration_minutes" type="number" required min={1} placeholder="60"
              className="w-full h-9 px-3 rounded-lg border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1.5">Prix (€)</label>
            <input name="prix" type="number" step="0.01" min="0" placeholder="256.00"
              className="w-full h-9 px-3 rounded-lg border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <div className="sm:col-span-2">
            <label className="block text-xs font-medium text-muted-foreground mb-1.5">Description *</label>
            <input name="product_title" required placeholder="Vol de découverte 60 min" defaultValue="Voucher vol sur mesure"
              className="w-full h-9 px-3 rounded-lg border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
        </div>
