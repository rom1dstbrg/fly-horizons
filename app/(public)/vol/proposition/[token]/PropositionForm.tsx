...
export function PropositionForm({
  ...
  // Verrouille le scroll body uniquement sur desktop (le layout fixe couvre tout l'écran)
  useEffect(() => {
    const mq = window.matchMedia("(min-width: 1024px)");
    const sync = () => { document.body.style.overflow = mq.matches ? "hidden" : ""; };
    sync();
  ...
  return (
    <>
      {/* ── Mobile (< lg) : layout scroll naturel ─────────────── */}
      <div className="lg:hidden flex flex-col gap-3 px-3 pt-[84px] pb-8">
        {/* Carte */}
        <div className="h-64 sm:h-72 relative rounded-[var(--r-sm)] overflow-hidden shadow-[0_2px_16px_rgba(0,0,0,0.10)] border border-border [isolation:isolate]">
