...
export function MessagesThread({ token, initialMessages }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    start(async () => {
      const r = await submitClientMessageReply(token, trimmed);
      if (r?.error) {
      ...
      {messages.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-sm text-muted-foreground leading-relaxed">
            ...
            Écrivez ici pour joindre votre pilote.
          </p>
        </div>
      ) : (
        <div>
          {messages.map((msg, i) => {
            ...
            const senderLabel = msg.author_nom?.trim() || "Fly Horizons";
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {isLast && (
                    <p className="text-[10px] text-muted-foreground mt-1 px-0.5">{timeStr}</p>
                  )}
                </div>
              </div>
            );
