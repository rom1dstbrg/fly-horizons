...
export default async function BonsPage() {
  ...
  const available = vouchers.filter((v) => v.status === "unused").length;

  if (vouchers.length === 0) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Mes bons de vol</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Aucun bon</p>
        </div>
